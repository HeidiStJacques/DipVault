  ...
  const fetchData = useCallback(async () => {
    try {
      const [vaultRes, productsRes] = await Promise.all([
        authFetch(`${API_BASE}/vaults/${id}`),
        authFetch(`${API_BASE}/products/?vault_id=${id}`),
      ]);

      if (vaultRes.status === 401 || productsRes.status === 401) {
        return;
      }
