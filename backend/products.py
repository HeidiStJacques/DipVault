...
class ProductCreate(BaseModel):
    ...
    # Keep this temporarily while the old single-photo
    # system still exists in the app.
    image_url: Optional[str] = None

    vault_ids: list[UUID] = Field(default_factory=list)
...
class ProductOut(BaseModel):
    ...
    vaults: list[VaultRef] = Field(default_factory=list)

    # New multi-photo system.
    photos: list[ProductPhotoOut] = Field(default_factory=list)
...
def resolve_vaults(
    ...
    """Fetch and validate that vaults belong to the current user."""
...
def list_products(
    ...
    if is_favorite is not None:
        query = query.filter(
            models.Product.is_favorite == is_favorite
        )
